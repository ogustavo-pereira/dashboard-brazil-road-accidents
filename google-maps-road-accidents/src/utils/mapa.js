...
function CarregarPontos(uf, horas) {
  ...
  if (horas != 24) {
    for (var x in teste) {
      const building = teste[x];
      if (building.horas == horas) {
        const location = new google.maps.LatLng(building.lat, building.long);
...
function CarregarPontoTodos() {
  for (let x in todospontos) {
    const building = todospontos[x];
    const location = new google.maps.LatLng(building.lat, building.long);
    const marker = new google.maps.Marker({
      position: location,
      icon: imagem,
